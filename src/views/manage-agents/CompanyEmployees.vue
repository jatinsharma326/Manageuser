    ...
    <div class="companyEmployeeWrapper">
        ...
        <div class="card-wrapper">
            ...
            >
                <InformationCard
                    :expandCard="true"
                    :isCardDisabled="!employee.record.active"
                >
                    ...
                    <template v-slot:actionButtons>
                        <template>
                            <v-btn
                                @click="openChangelogsModal(employee)"
                                icon
                                color="secondary"
                                text
                    ...
                    <template v-slot:expandCardContent>
                        <v-list>
                            ...
                            <v-list-item
                                v-for="(
                                    contact, index
                                ) in employee.landline_numbers"
                                :key="
                                    employee._id +
                                    '+' +
                                    index +
                                    '+' +
                                    employee.landline_numbers
                                "
                            >
                                <v-list-item-icon>
                                    <v-icon color="secondary">
            ...
            if (
                ...
                this.editCompanyEmployee({
                    ...
                    if (data.ok) {
                        this.openSnackbar({
                            text:
                                "Sucessfully Updated Travel Agent Employee Status",
                        });
                        this.getData();
                    } else {
                        this.openSnackbar({ text: data.message });
    ...
    watch: {
        companyInfo: {
            ...
            async handler(nv, ov) {
                ...
                this.setInputConfig(
                    this.addressList,
                    this.companyInfo.business_types
                );
                await this.getStates();
                this.setSearchConfig(this.statesList);
                // if (nv.countries) {
