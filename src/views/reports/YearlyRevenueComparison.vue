...
export default {
  ...
  methods: {
    ...mapActions("Reports", [
      "getYearlyComparison",
      "downloadYearlyComparisonReport",
    ]),
    ...mapMutations([]),
    setDateRange() {
      let tempArray = [];
    ...
    isDateValid(newValue) {
      ...
      if (diffrenceInStartDates == 0 && diffrenceInEndDates == 0) {
        this.dateErrorMessage =
          "Date range can't be the same as main date Range";
        return false;
      }
      return true;
    },
    getData() {
      ...
      let selectionDate = JSON.parse(
        JSON.stringify(this.yearlyRevenueMainDate)
      );
      selectionDate.sort();
