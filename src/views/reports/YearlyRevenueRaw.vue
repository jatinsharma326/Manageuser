    ...
    <div v-else class="leaves-table">
      ...
      >
        ...
        <template v-slot:[`item.date_of_travel`]="{ item }">
          {{
            item.date_of_travel
              ? getFormattedDate(item.date_of_travel, "MMMM Do YYYY dddd")
              : "-"
          }}
        </template>
        <template v-slot:[`item.reminder_date`]="{ item }">
          {{
            item.reminder_date
              ? getFormattedDate(item.reminder_date, "MMMM Do YYYY dddd")
              : "-"
          }}
        </template>
...
export default {
  ...
  methods: {
    ...
    ...mapMutations("Reports", [
      "setYearlyRevenueMainDate",
      "setYearlyRevenueFilter",
      ,
    ]),
    setDateRange() {
      let tempArray = [];
      let startDate = moment()
        ...
        .startOf("year")
        .format("YYYY-MM");
      let endDate = moment().tz("Asia/Kolkata").endOf("year").format("YYYY-MM");
    ...
    isDateValid() {
      ...
      if (this.datePickerDate.length == 2) {
        let diffrenceInDates = moment(this.datePickerDate[1])
          .tz("Asia/Kolkata")
          .diff(
            moment(this.datePickerDate[0]).tz("Asia/Kolkata"),
            "months",
            true
          );
