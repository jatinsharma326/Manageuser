...
export default {
	...
	actions: {
		...
		getPolicyStatus: ({ commit, dispatch }) => {
			let fail = (msg) => commit("failure", msg);
			return dispatch(
				"apiCall",
				{
					method: "get",
					params: {},
					url: constants.CHECK_POLICIES_STATUS,
				},
				{ root: true }
			)
				.then((data) => {
					if (data.ok) {
						return {
							ok: true,
							policyStatus: data.isPolicy,
						};
					} else {
						fail(data.message || "Failed to check Policy File Status");
						return {
							ok: false,
							policyStatus: false,
						};
				...
				.catch((err) => {
					console.log("Yo ", err);
					fail(err.toString() || "Failed to check Policy File Status");
					return {
						ok: false,
						policyStatus: false,
					};
		...
		updatePasswordAPI: ({ commit, dispatch }, payload) => {
			let fail = (msg) => commit("failure", msg);
			return dispatch(
				"apiCall",
				{
				...
				.then((data) => {
					if (!data.ok) fail(data.message || "Failed to UPDATE PASSWORD");
					return data;
				})
				.catch((err) => {
					fail(err.toString() || "Failed to UPDATE PASSWORD");
					return {
						ok: false,
						message: err.message,
					};
