      ...
      <div class="form-input">
        <template v-for="(config, index) in inputConfig">
          <v-text-field
            ...
            :disabled="
              disableConfig[config.key] ? disableConfig[config.key] : false
            "
          ></v-text-field>
          ...
          <template v-else-if="config.type == 'DropdownWithMoreInfo'">
            ...
            >
              <template v-slot:[`item`]="{ item }">
                <v-list-item-content>
                  <v-list-item-title
                    v-text="config.titleContent(item)"
                  ></v-list-item-title>
                  <v-list-item-subtitle
                    v-text="
                      config.subtitleContent ? config.subtitleContent(item) : ''
                    "
                  ></v-list-item-subtitle>
                </v-list-item-content>
              </template>
          ...
          >
            <template v-for="(row, rowIndex) in formElements[config.key]">
              <div
                :key="config.name + '__' + index + '__' + rowIndex"
                class="input-with-group-key-title"
              >
    ...
    keyUpdated(watchKey, nv, ov) {
      for (let i of this.inputConfig) {
        if (i.type == "MultiInputWithGroupKey" && watchKey == i.keyToGroup) {
          ...
          i.type == "AsyncDropdownWithMoreInfo" &&
          watchKey == i.triggerKey
        ) {
          if (nv) {
